...
3. **Pre-flight.** Dispatch the `@explore` subagent on `$1` to return acceptance criteria, the Plan U-ID steps, the coding standards that apply, the ADRs in play, and any relevant CLAUDE.md §10 pitfalls.
4. **Move.** Run: `python tools/board/ticket_move.py $1 in-progress --note "Starting execution"`
5. **Brief me** with the plan summary and wait for my go-ahead before writing any code.
