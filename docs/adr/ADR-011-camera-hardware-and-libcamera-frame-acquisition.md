...
### D6 — Infrared wavelength is 850 nm

The IR laser referred to by ADR-005 and ADR-008 is specified as **850 nm**. Silicon sensor quantum efficiency at 940 nm is substantially lower than at 850 nm, and the modulation-correlation window of ADR-005 depends on a usable signal-to-noise ratio at the laser spot. 850 nm emits a faint visible red glow at the aperture, which is accepted: with children present, a laser that is entirely invisible is harder to notice and avoid than one that is faintly visible.

No IR band-pass filter is fitted. The same camera must detect the ball in ambient light and the laser spot in IR; a band-pass filter would optimise the second at the cost of the first.
