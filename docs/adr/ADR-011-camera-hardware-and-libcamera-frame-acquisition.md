...
### Gate measurements (appended as taken; this ADR stays `Proposed` until all pass)

**G1 — measured 2026-07-30 on the Pi 5 with the fitted module (TRK-034 U2). Passes as written, but surfaces an open question against D4.** The camera enumerates as `imx708_wide_noir` and the 1536×864 mode exists with a 120.13 fps ceiling, so both 60 and 120 fps are admitted — G1's literal criterion is met. The measurement also shows something G1's wording did not ask for: **the 1536×864 mode is a centred 3072×1728 crop of the 4608×2592 array — 2/3 of the sensor linearly — whereas the 102° H / 67° V figures in D2 are full-array numbers.** Only the 2304×1296 mode reads the full array, and it caps at 56.03 fps, below the §5 ≥60 fps ship gate.
...
- **Frame-rate versus mode assumption is unverified.** D4 assumes IMX708 permits the 1536×864 mode at an extended frame duration while retaining its short readout. This is standard sensor behaviour but is asserted, not measured. *Mitigation:* gate G1, then G4. **Partially discharged 2026-07-30:** the frame-rate half is confirmed (120.13 fps ceiling on that mode); the readout-time half still awaits G4.
- **D4's mode selection costs field of view — measured, and not anticipated when D4 was written.** The 1536×864 mode is a 2/3-linear centre crop, so selecting it for readout time also crops coverage to roughly 79° H against D2's assumed 102° H, and the only full-array mode caps at 56.03 fps. D4 claims the mode choice is "at no CPU cost and with no code", which remains true, but it is **not free** — the cost is coverage, paid against the exact criterion on which D2 rejected the Standard variant. *Mitigation:* gate G2, shot in the 1536×864 mode, decides whether the working area still fits. If it does not, D4 and D2 are jointly in tension and one of them must give — the candidate resolutions are accepting ~56 fps on the full-array mode (which breaches the §5 ship gate), accepting the cropped coverage (which may reproduce the failure D2 rejected the Standard for), a wider lens, or the global-shutter escalation already named in Alternatives. Recorded 2026-07-30; unresolved.
