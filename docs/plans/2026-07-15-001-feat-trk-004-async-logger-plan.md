...
- **KTD-2 — overflow policy `async_overflow_policy::overrun_oldest`.** Never block the producer waiting for queue space; drop the oldest queued message instead. Same staleness philosophy as ADR-002's `ZMQ_CONFLATE=1`. Consequence accepted: a burst can drop WARN/ERROR messages; 8192 slots makes that rare, and the alternative (blocking the hot path on a full queue) violates §7.1. Precision from review: spdlog's queue is a `std::mutex`-protected ring, not strictly lock-free — the enqueue takes a short critical section shared with the worker. Acceptable for v0.3's threads; TRK-006's SCHED_FIFO capture thread must not adopt `LOG_*` in its loop without a priority-inversion decision. spdlog is pinned ≥ v1.15 because v1.14 heap-allocated a `std::promise` per message, violating R3.
